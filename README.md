...
- **Tailwind CSS 4**: Framework CSS untuk desain modern dan responsif.
- **JavaScript (Vanilla)**: Logika interaktif dan animasi.
- **Vite**: Build tool untuk workflow pengembangan yang cepat.
- **Lucide Icons**: Library ikon vektor yang modern.
...
Project ini menggunakan **Track B (Vite + Tailwind CSS)**. Ikuti langkah berikut untuk menjalankan di komputer lokal:
...
4. Jalankan project dalam mode development:
   ...
   npm run dev
   ```
5. Buka alamat lokal yang muncul di terminal (biasanya `http://localhost:5173/`) di browser Anda.

## Refleksi Penggunaan AI
Saya menggunakan AI (Antigravity) untuk membantu:
- Memahami konfigurasi terbaru **Tailwind CSS 4** dan integrasinya dengan **Vite**.
- Menyusun struktur grid dan flexbox yang kompleks agar tampilan tetap rapi di semua ukuran layar.
- Menemukan solusi untuk error PostCSS terkait urutan `@import` di file CSS.
- Membuat logika JavaScript untuk animasi *reveal on scroll* menggunakan *Intersection Observer*.
...
- Saya dapat menjelaskan cara kerja class Tailwind untuk membuat layout yang responsif (`grid-cols-1 md:grid-cols-2`).
